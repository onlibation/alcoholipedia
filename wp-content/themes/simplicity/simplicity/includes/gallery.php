...
if ( !empty($attachments) ) :
	...
	foreach ( $attachments as $att_id => $attachment ) {
		...
		if ( ( get_option('woo_resize') == "true" && $use_timthumb == "true" ) || ( $src[1] > 540 ) )
  			$photo_output .= '<div><a href="'. $src[0] .'" rel="prettyPhoto" title="'.$attachment->post_excerpt.'">'.woo_image('src=' . $src[0] . '&key=image&width=' . $width_setting . '&link=img&return=false&alt=' . $caption . '&class=single-photo').'</a></div>';
		else
  			$photo_output .= '<div' . $style . '><a href="'. $src[0] .'" rel="prettyPhoto" title="'.$attachment->post_excerpt.'"><img src="'. $src[0] .'" class="single-photo" alt="'.$attachment->post_excerpt.'" /></a>'.$caption.'</div>'; 
